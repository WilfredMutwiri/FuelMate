  ...
  const handleReassign = async () => {
    try {
      const res = await fetch(
        `${SERVER_URL}/api/v1/order/emergency/${id}/reassign`,
        {
          method: "PATCH",
        }
      );
    ...
    <div>
      <div className="w-11/12 mx-auto bg-white">
        <div className="flex justify-center border border-cyan-300">
          ...
          <h2 className="text-2xl font-semibold my-auto text-cyan-800">
            Fuel Mate
          </h2>
        </div>
        <div>
          ...
          <div className="flex justify-between p-3 gap-6 w-11/12 mx-auto mt-4">
            <div>
              <ul className="flex flex-col gap-3">
                ...
                <li>
                  Fuel Type Requested:{" "}
                  <span className="demographyLi">{Order?.order?.fuelType}</span>
                </li>
                <li>
                  Volume Requested:{" "}
                  <span className="demographyLi">
                    {Order?.order?.fuelVolume} L
                  </span>
                </li>
                <li>
                  Order Status:{" "}
                  <span className="demographyLi">{Order?.order?.status}</span>
                </li>
                <li>
                  Urgency:{" "}
                  <span className="demographyLi">{Order?.order?.urgency}</span>
                </li>
              </ul>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <p className="border-2 p-5 rounded-md">
                  <span className="demographyLi text-base">
                    {Order?.order?.message}
                  </span>
                </p>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-row gap-10">
                <div>
                  <img
                    src={Order?.order?.assignedStation?.profileImg}
                ...
                <div>
                  <ul className="flex flex-col gap-2">
                    <li>
                      Station Name:{" "}
                      <span className="demographyLi">
                        {Order?.order?.assignedStation?.stationName}
                      </span>
                    </li>
                    <li>
                      Station Phone No:{" "}
                      <span className="demographyLi">
                        {Order?.order?.assignedStation?.phoneNo}
                      </span>
                    </li>
                    <li>
                      Station Location:{" "}
                      <span className="demographyLi">
                        {Order?.order?.assignedStation?.county} -{" "}
                        {Order?.order?.assignedStation?.town}
                      </span>
                    </li>
                    <li>
                      Station Email:{" "}
                      <span className="demographyLi">
                        {Order?.order?.assignedStation?.email}
                      </span>
                    </li>
                  </ul>
                </div>
          ...
          <div className="w-11/12 mx-auto pt-10 flex gap-6">
            <Button color={"warning"} onClick={handleReassign}>
              Re-Assign
            </Button>
          </div>
          ...
          <div className="mx-auto pt-5 flex gap-10 overflow-x-auto w-full">
            {Order.order.assignmentHistory.length > 0 &&
              Order.order.assignmentHistory
                ?.sort(
                  ...
                  <div key={order._id} className="pt-10 min-w-[320px] shrink-0">
                    ...
                    <ul className="pt-5">
                      ...
                      <li>
                        Phone No:{" "}
                        <span className="demographyLi">
                          {order?.station?.phoneNo}
                        </span>
                      </li>
                      <li>
                        Location:{" "}
                        <span className="demographyLi">
                          {order?.station?.county} - {order?.station?.town}
                        </span>
                      </li>
                      <li>
                        Status:{" "}
                        <span className="demographyLi">{order?.status}</span>
                      </li>
                    </ul>
                  </div>
                ))}
