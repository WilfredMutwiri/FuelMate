                ...
                <div className="flex-1 gap-4 mt-4">
                    ...
                    <div className={`bg-gray-200 p-1 rounded-md overflow-hidden ${showAll?"h-[500px]":"h-auto"}`}>
                        <div className='flex justify-between bg-gray-200 rounded-md p-2'>
                            <h2 className="mx-auto p-2 text-left text-lg text-green-700">Accepted Emergency Orders <span className='pl-10 text-green-700'> ( {emergencyOrders?.length} )</span></h2>
                        </div>
                                ...
                                <Table.Body className="divide-y">
                                    {emergencyOrders?.length >0 && emergencyOrders?.map((order) => (
                                        <Table.Row key={order._id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <Table.Cell className="text-black">
                                                <Link to={`/order/${order._id}`} className="font-medium text-green-700 hover:text-red-600 hover:underline">View</Link>
                                            </Table.Cell>
                                        </Table.Row>
                                    ))}
                                </Table.Body>
