...
const getAllApprovedStations = async (req, res) => {
  try {
    const stations = await Station.find({ status: "Approved" }).sort({
      createdAt: -1,
    });
    return res.status(200).json({
      message: "Stations fetched successfully",
...
const getAllNotApprovedStations = async (req, res) => {
  try {
    const stations = await Station.find({ status: "Not Approved" }).sort({
      createdAt: -1,
    });
    return res.status(200).json({
      message: "Stations fetched successfully",
...
const addStationService = async (req, res) => {
  ...
  try {
    ...
    const updatedServices = Array.from(
      new Set([
        ...currentServices,
        ...services.map((s) => s.trim().toLowerCase()),
      ])
    );
...
const likeStation = async (req, res) => {
  ...
  try {
    ...
    station.dislikedBy = station.dislikedBy.filter(
