...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    const statusMessages = {
      ...
      approved:
        "Good news! Your order has been approved and will be dispatched soon.",
      delivered:
        "Your fuel has been delivered. Thank you for choosing FuelMate!",
      canceled:
        "Your order has been canceled. Please contact support if this is unexpected.",
    };
    // notification
...
const getTotalVolumeDeliveredByStation = async (req, res) => {
  ...
  try {
    ...
    const totalVolume = deliveredOrders.reduce(
      (sum, order) => sum + order.fuelVolume,
      0
    );

    return res.status(200).json({
      message:
        "Total fuel volume delivered by this station calculated successfully!",
      totalVolume,
      success: true,
    });
  } catch (error) {
